...
from _helpers import (formatdate, readdemoheader, convertunit,
	formatbookmarkdata)

from _dialogues import (CfgError, Cleanup, Deleter, FirstRunDialog,
	LaunchTF2, Settings)
from _threads import ThreadDeleter, ThreadFilter, ThreadReadFolder
...
class MainApp():
	def __init__(self):
		...
		#	("Insert bookmark", " into selected demo...", self.__insertbookmark), )

		self.cfgpath = os.path.join(os.getcwd(), _DEF.CFG_FOLDER, _DEF.CFG_NAME)
	...
	def __startup(self):
		...
		if os.path.exists(self.cfgpath):
			...
			try:
				...
				self.writecfg(_DEF.DEFAULT_CFG)
	...
	def __cleanup(self):
		...
		dialog_ = Cleanup(self.mainframe, {"cfg":self.cfg,
			"curdir":self.curdir, "bookmarkdata":self.bookmarkdata,
	...
	def __deldem(self):
		...
		if index == None:
			...
			return
		filename = self.listbox.getcell("col_filename", index)
		dialog = Deleter(self.root, {"cfg":self.cfg, "files":[filename],
			"selected": [True], "demodir":self.curdir, "keepeventsfile": False,
			"deluselessjson":False, "styleobj":self.ttkstyle} )
...
#		dialog_ = BookmarkSetter(self.root, {"targetdemo":path})
	...
	def __applytheme(self):
		...
		if self.cfg["ui_theme"] == "_DEFAULT":
			...
			return
		theme_subdir = _DEF.THEME_SUBDIR
		try:
			theme_tuple = _DEF.THEME_PACKAGES[self.cfg["ui_theme"]]
	...
	def __after_callback_fetchdata(self):
		...
		if not finished:
			self.after_handlers["datafetcher"] = self.root.after(
				_DEF.GUI_UPDATE_WAIT, self.__after_callback_fetchdata)
		else:
			self.root.after_cancel(self.after_handlers["datafetcher"])
	...
	def __after_callback_filter(self):
		...
		if not finished:
			self.after_handlers["filter"] = self.root.after(
				_DEF.GUI_UPDATE_WAIT, self.__after_callback_filter)
		else:
			self.__stopfilter()
	...
	def setstatusbar(self, data, timeout = None):
		...
		if timeout != None:
			self.after_handlers["statusbar"] = self.statusbarlabel.after(
				timeout, lambda: self.setstatusbar(_DEF.STATUSBARDEFAULT) )
	...
	def writecfg(self, data):
		...
		while not write_ok:
			...
			try:
				...
				if handleexists:
					handle.close()
				dialog = CfgError(self.root, self.cfgpath, error, 1)
	...
	def getcfg(self):
		...
		localcfg = _DEF.DEFAULT_CFG.copy()
		cfg_ok = False
		while not cfg_ok:
			handleexists = False
			try:
				...
				for k in _DEF.DEFAULT_CFG_TYPES:
					if k == "demopaths":
					...
					if not _DEF.DEFAULT_CFG_TYPES[k] == type(localcfg[k]):
						raise ValueError("Bad config type: {} is not of type \"{}\"".format(localcfg[k].__repr__(), _DEF.DEFAULT_CFG_TYPES[k].__name__) )
				cfg_ok = True
			except (json.decoder.JSONDecodeError, FileNotFoundError, ValueError, OSError) as error:
				if handleexists:
					cfghandle.close()
				dialog = CfgError(self.root, self.cfgpath, error, 0)
				if dialog.result_ == 0: # Retry
					pass
				elif dialog.result_ == 1: # Config replaced by dialog
					pass
