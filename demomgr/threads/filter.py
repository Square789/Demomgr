...
class ThreadFilter(_StoppableBaseThread):
	...
	def run(self):
		...
		try:
			filters, flags = process_filterstring(filterstring)
		...
		for i, j in enumerate(demo_data["col_filename"]): # Filter
			...
			tmp_di = ((), ()) if demo_data["col_demo_info"][i] == None else demo_data["col_demo_info"][i]
			...
			if flags & FILTERFLAGS.HEADER:
				try:
					curdataset["header"] = readdemoheader(os.path.join(curdir, j))
