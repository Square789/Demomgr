...
class Play(BaseDialog):
	...
	def body(self, master):
		...
		rcon_password_entry = ttk.Entry(rcon_password_frame, style = "Contained.TEntry")
		self.rcon_connect_button = ttk.Button(rcon_labelframe, style = "Contained.TButton")
		self.rcon_text = TtkText(rcon_labelframe, self._style, height = 4, width = 48)
		self.rcon_text.mark_set("spinner", "1.9")
		...
		self.user_select_combobox = ttk.Combobox(
			launch_config_frame, textvariable = self.userselectvar, state = "readonly"
		)
		self.use_hlae_checkbox = ttk.Checkbutton(
			play_frame, variable = self.usehlae_var, text = "Use HLAE",
			style = "Contained.TCheckbutton"
		)

		arg_region = ttk.Frame(play_frame, style = "Contained.TFrame")
		self.launch_options = ttk.Entry(arg_region, style = "Contained.TEntry")
		self.launch_commands = ttk.Entry(arg_region, style = "Contained.TEntry")
		...
		self.rcon_text.grid(row = 0, column = 1, rowspan = 2)
		rcon_labelframe.grid(row = 0, column = 0, sticky = "nesw")

		# = Play frame
		# Launch config region
		user_select_label.grid(row = 0, column = 0)
		self.user_select_combobox.grid(row = 0, column = 1)
		self.use_hlae_checkbox.grid(row = 1, column = 0)
		launch_config_frame.grid(row = 0, column = 0)

		# Launch arg region
		self.launch_options.grid(row = 1, column = 0)
		self.launch_commands.grid(row = 2, column = 0)
		self.rcon_send_commands_button.grid(row = 2, column = 1)
		arg_region.grid(row = 1, column = 0)

		# Event tick region
		self.tick_mfl.grid(row = 0, column = 0, rowspan = 3)
		self.tick_to_launch_options_checkbox.grid(row = 0, column = 1)
		self.tick_entry.grid(row = 1, column = 1)
		self.rcon_send_gototick_button.grid(row = 2, column = 1)
		bookmark_region.grid(row = 2, column = 0)
	...
	def _getlaunchoptions(self):
		try:
			tmp = self.user_select_combobox.current()
			if tmp == -1:
				raise KeyError("Bad or empty (?) steam folder")
			user_id = self.users[tmp][0]
			with open(os.path.join(
					self.cfg.steam_path, CNST.STEAM_CFG_PATH0, user_id, CNST.STEAM_CFG_PATH1
				), encoding = "utf-8"
			) as h:
				subelem = vdf.load(h)
	...
	def _rcon_txt_set_line(self, n, content):
		...
		"""
		with self.rcon_text:
			self.rcon_text.delete(f"{n + 2}.0", f"{n + 2}.{tk.END}")
			self.rcon_text.insert(f"{n + 2}.0", content)

	def _rcon(self):
		self.rcon_connect_button.configure(text = "Cancel", command = self._cancel_rcon)
		self.rcon_text.configure(state = tk.NORMAL)
		for i in range(3):
			self._rcon_txt_set_line(i, "")
		self.rcon_text.configure(state = tk.DISABLED)
	...
	def _rcon_after_run_always(self):
		with self.rcon_text:
			self.rcon_text.delete("spinner", "spinner + 1 chars")
			self.rcon_text.insert("spinner", next(self.spinneriter))
	...
	def done(self, launch=False):
		...
		self.result.remember = [
			...
			self.users[self.user_select_combobox.current()][0] \
				if self.user_select_combobox.current() != -1 else ""
		]
